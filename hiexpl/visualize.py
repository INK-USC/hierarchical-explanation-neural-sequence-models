...
def compact_layer(layers):
    ...
    for i, layer in enumerate(layers):
        if i != len(layers) - 1 and len(layers[i]) == 1 and len(layers[i+1]) == 1 and not skip_flg:
        ...
        for i in range(score_array.shape[0]):
            for j in range(score_array.shape[1]):
                if score_array[i,j] != 0:
                    if sent_words[j].startswith('SUBJ') or sent_words[j].startswith('OBJ'):
                        sent_words[j] = sent_words[j].replace('SUBJ-','S-').replace('OBJ-','O-').upper()
                    fontsize = 12
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_gt_trees', help='whether use ground truth parsing trees (sst)'
                                               'if true, --file should be .txt', action='store_true')
