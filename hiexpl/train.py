...
def do_train():
    ...
    if args.optim == 'adam':
        opt = O.Adam(filter(lambda x: x.requires_grad, model.parameters()), weight_decay=1e-6, lr=args.lr)
    else:
        opt = O.SGD(filter(lambda x: x.requires_grad, model.parameters()), weight_decay=1e-6, lr=args.lr)
    iterations = 0
    start = time.time()
    best_dev_acc = -1
    prev_dev_acc = -1
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_iter):
            ...
            n_correct += (torch.max(answer, 1)[1].view(label.size()).data == label.data).sum()
            ...
            if iterations % args.dev_every == 0:
                ...
                with torch.no_grad():
                    for dev_batch_idx, dev_batch in enumerate(dev_iter):
                        ...
                        dev_label = dev_batch.label if not config.use_gpu else dev_batch.label.cuda()
                        pred = torch.max(answer, 1)[1]
                        n_dev_correct += (pred.view(dev_label.size()).data == dev_label.data).sum()
                ...
                if args.metrics == 'tacred_f1':
                    dev_acc = 100. * tacred_f1_score(truth_dev, pred_dev)[-1]
                else:
                ...
                if dev_acc > best_dev_acc:
                    ...
                    snapshot_path = snapshot_prefix + '_devacc_{}_devloss_{}_iter_{}_model.pt'.format(dev_acc,
                                                                                                      dev_loss.item(),
                                                                                                      iterations)
...
def do_test():
    ...
    for batch_idx, batch in enumerate(test_iter):
        ...
        if batch_idx % 100 == 0:
            print('evaluating %d\tacc: %f, %d, %d' % (batch_idx, acc, n_correct, n_total))
...
if __name__ == '__main__':
    ...
    if args.task == 'sst':
        ...
        inputs, labels, train_iter, dev_iter, test_iter, train_set, dev_set = \
            get_data_iterators_yelp(map_cpu=False)
    elif args.task == 'tacred':
        inputs, labels, subj_offset, obj_offset, pos, ner, train_iter, dev_iter, test_iter, train_set, dev_set = get_data_iterators_tacred()
