...
def lr_gt_pearson():
    ...
    model = torch.load(args.bow_snapshot, map_location='cpu')
    dict_path, label_path = '.data/sst_raw/dictionary.txt', '.data/sst_raw/sentiment_labels.txt'
    if BERT:
...
def run_multiple(path):
    ...
    f = open('analysis/%s_%s' % (TASK,template.split('/')[-1].replace('{','').replace('}','') + postfix),'w')
