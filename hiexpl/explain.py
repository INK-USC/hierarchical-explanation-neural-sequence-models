...
from utils.reader import get_data_iterators_sst_flatten, get_data_iterators_yelp, get_data_iterators_tacred
import random, os
...
if __name__ == '__main__':
    ...
    if args.task == 'sst' or args.task == 'sst_async':
        text_field, length_field, train_iter, dev_iter, test_iter, train, dev = \
        ...
        text_field, label_field, subj_offset, obj_offset, pos, ner, train_iter, dev_iter, test_iter, train, dev = \
    ...
    if args.explain_model == 'lstm':
        ...
        if args.method == 'soc':
            ...
            algo = SOCForLSTM(model, lm_model, iter_map[args.dataset],
                              tree_path=tree_path % args.dataset, config=args,
                              vocab=text_field.vocab,
                              output_path='outputs/' + args.task + '/soc_results/soc%s.txt' % args.exp_name)
        elif args.method == 'scd':
            lm_model = torch.load(args.lm_path, map_location=lambda storage, location: storage.cuda(args.gpu))
            lm_model.gpu = args.gpu
            lm_model.encoder.gpu = args.gpu
            algo = SCDForLSTM(model, lm_model, iter_map[args.dataset],
                              tree_path=tree_path % args.dataset, config=args,
                              vocab=text_field.vocab,
                              output_path='outputs/' + args.task + '/scd_results/scd%s.txt' % args.exp_name)
        else:
        ...
        model = BertForSequenceClassification(config, num_labels=2 if args.task != 'tacred' else 42)
        ...
        if args.method == 'soc':
            ...
            lm_model.gpu = args.gpu
            lm_model.encoder.gpu = args.gpu
            algo = SCDForTransformer(model, lm_model, tree_path=tree_path % args.dataset,
