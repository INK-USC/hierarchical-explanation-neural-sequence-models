...
class LSTMLanguageModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        fw_loss = self.loss(fw_proj[:-1].view(-1,fw_proj.size(2)).contiguous(), inp[1:].view(-1).contiguous())
        bw_loss = self.loss(bw_proj[1:].view(-1,bw_proj.size(2)).contiguous(), inp[:-1].view(-1).contiguous())
        return fw_loss, bw_loss

    def _sample_n_sequences(self, method, direction, token_inp, hidden, length, sample_num):
        outputs = []
        token_inp = token_inp.repeat(1, sample_num) # [1, N]
        hidden = hidden[0].repeat(1, sample_num, 1), hidden[1].repeat(1, sample_num, 1) # [x, N, H]
    ...
    def sample_n(self, method, batch, max_sample_length, sample_num):
        ...
        padded_enc_out, (padded_hidden_states, padded_cell_states) = self.encoder(padded_inp, inp_len_np + 2,
                                                                                  return_all_states=True)  # [T+2,B,H]
        ...
        fw_sample_outputs = self._sample_n_sequences(method, 'fw', fw_next_token, (fw_hidden_state, fw_cell_state),
                                                     max_sample_length, sample_num)
        bw_sample_outputs = self._sample_n_sequences(method, 'bw', bw_next_token, (bw_hidden_state, bw_cell_state),
                                                     max_sample_length, sample_num)
