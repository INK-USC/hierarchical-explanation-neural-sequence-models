...
def get_data_iterators_sst():
    ...
    train, dev, test = tt.datasets.SST.splits(inputs, answers, fine_grained=False, train_subtrees=not args.no_subtrees,
    ...
    if args.include_noise_labels:
        train_2, _, _ = tt.datasets.SST.splits(inputs, answers, fine_grained=False, train_subtrees=True)
        texts = set()
        ...
        train_iter, dev_iter, test_iter = tt.data.BucketIterator.splits(
            (train, dev, test), batch_size=args.batch_size, device=args.gpu, sort=True, shuffle=False)
    else:
        train_iter, dev_iter, test_iter = tt.data.BucketIterator.splits(
