from algo.soc_transformer import ExplanationBaseForTransformer, SOCForTransformer, normalize_logit
from bert.run_classifier import BertTokenizer, predict_and_explain_wrapper_unbatched
from algo.soc_transformer import get_data_iterator_bert, bert_id_to_lm_id, lm_id_to_bert_id, Batch
...
class CDForTransformer(ExplanationBaseForTransformer):
    def __init__(self, model, tree_path, output_path, config):
        super().__init__(model, tree_path, output_path, config)
        self.model = model
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True, cache_dir='bert/cache')
        self.tree_path = tree_path
        self.max_seq_length = 128
        self.batch_size = config.batch_size
        self.iterator = get_data_iterator_bert(self.tree_path, self.tokenizer, self.max_seq_length, self.batch_size)
    ...
    def explain_single_transformer(self, input_ids, input_mask, segment_ids, region, label=None):
        ...
        if not args.task == 'tacred':
            score = predict_and_explain_wrapper_unbatched(self.model, input_ids, segment_ids, input_mask, region)
        else:
            score = predict_and_explain_wrapper_unbatched(self.model, input_ids, segment_ids, input_mask, region,
                                                          normalizer=normalize_logit, label=label)
        return score


class SCDForTransformer(SOCForTransformer):
    def __init__(self, target_model, lm_model, vocab, tree_path, output_path, config):
        super().__init__(target_model, lm_model, vocab, tree_path, output_path, config)
        if args.cd_pad:
            self.sample_num = 1
    ...
    def get_states(self, inp, inp_mask, segment_ids, x_regions, nb_regions):
        ...
        if not args.task == 'tacred':
            ...
            inp_th = torch.from_numpy(
                bert_id_to_lm_id(inp_lm[:inp_length], self.tokenizer, self.vocab)).long().view(-1, 1)
            ...
            for sample_i in range(self.sample_num):
                ...
                if len_bw > 0:
                    filled_inp[nb_region[0]:x_region[0]] = lm_id_to_bert_id(bw_sample_seq[-len_bw:], self.tokenizer,
                                                                            self.vocab)
                if len_fw > 0:
                    filled_inp[x_region[1] + 1:nb_region[1] + 1] = lm_id_to_bert_id(fw_sample_seq[:len_fw],
                                                                                    self.tokenizer,
                                                                                    self.vocab)
