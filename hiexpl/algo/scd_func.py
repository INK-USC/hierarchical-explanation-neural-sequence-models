...
def CD_gpu(batch, model, intervals, hist_states, gpu):
    ...
    T = word_vecs.size(0)
    relevant, irrelevant, relevant_h, irrelevant_h = torch.zeros(T, hidden_dim).to(gpu),torch.zeros(T, hidden_dim).to(gpu),\
                                                     torch.zeros(T, hidden_dim).to(gpu),torch.zeros(T, hidden_dim).to(gpu)
    W_ii, W_if, W_ig, W_io = torch.split(lstm_module.weight_ih_l0, lstm_module.hidden_size)
    W_hi, W_hf, W_hg, W_ho = torch.split(lstm_module.weight_hh_l0, lstm_module.hidden_size)
    b_i, b_f, b_g, b_o = torch.split(lstm_module.bias_ih_l0 + lstm_module.bias_hh_l0, lstm_module.hidden_size)
    ...
    for i in range(T):
        ...
        rel_contrib_i, irrel_contrib_i, bias_contrib_i = decomp_activation_three_with_states(rel_i, irrel_i, b_i, F.sigmoid, hist_states, 'i', i, bias_in_rel)
        rel_contrib_g, irrel_contrib_g, bias_contrib_g = decomp_activation_three_with_states(rel_g, irrel_g, b_g, F.tanh, hist_states, 'g', i, bias_in_rel)

        i_states.append(sum([rel_i, irrel_i, b_i]))
        g_states.append(sum([rel_g, irrel_g, b_g]))

        relevant[i], irrelevant[i] = mult_terms(rel_contrib_g, irrel_contrib_g, bias_contrib_g, rel_contrib_i,
                                                irrel_contrib_i, bias_contrib_i, hist_states, 'act_g', 'act_i', i, bias_in_rel)

        if i > 0:
            rel_contrib_f, irrel_contrib_f, bias_contrib_f = decomp_activation_three_with_states(rel_f, irrel_f, b_f,
                                                                                                 F.sigmoid, hist_states,
                                                                                                 'f', i, bias_in_rel)

            rel_plus, irrel_plus = mult_terms(rel_contrib_f, irrel_contrib_f, bias_contrib_f, relevant[i-1],
                                              irrelevant[i-1], 0, hist_states, 'act_f', 'temp_c', i, bias_in_rel)
            relevant[i] += rel_plus
        ...
        rel_contrib_o, irrel_contrib_o, bias_contrib_o = decomp_activation_three_with_states(rel_o, irrel_o, b_o, F.sigmoid, hist_states, 'o', i, bias_in_rel)
        new_rel_h, new_irrel_h = decomp_activation_two_with_states(relevant[i], irrelevant[i], F.tanh, hist_states, 'c', i)
        c_states.append(sum([relevant[i], irrelevant[i]]))
        o_states.append(o)
        relevant_h[i], irrelevant_h[i] = mult_terms(rel_contrib_o, irrel_contrib_o, bias_contrib_o, new_rel_h,
                                                    new_irrel_h, 0, hist_states, 'o', 'tanhc', i, bias_in_rel)

    W_out = model.hidden_to_label.weight.data

    if hasattr(model, 'drop'):
        relevant_h[T-1], irrelevant_h[T-1] = model.drop(relevant_h[T-1]), model.drop(irrelevant_h[T-1])
...
def get_lstm_states(batch, model, gpu):
    ...
    hidden_dim = model.hidden_dim
    T = word_vecs.size(0)
    prev_c, prev_h = torch.zeros(batch_size, hidden_dim).to(gpu), torch.zeros(batch_size, hidden_dim).to(gpu)
    W_ii, W_if, W_ig, W_io = torch.split(lstm_module.weight_ih_l0, lstm_module.hidden_size)
    W_hi, W_hf, W_hg, W_ho = torch.split(lstm_module.weight_hh_l0, lstm_module.hidden_size)
    b_i, b_f, b_g, b_o = torch.split(lstm_module.bias_ih_l0 + lstm_module.bias_hh_l0, lstm_module.hidden_size)
    ...
    for ts in range(T):
        ...
        o = sigmoid(torch_mul(W_io, word_vecs[ts]) + torch_mul(W_ho, prev_h) + b_o.view(1,-1))
        c_states.append(c)
...
def decomp_activation_three_with_states(rel_x, irrel_x, bias_x, activation, states, state_key, t, bias_in_rel):
    rel = 0
    if states is None or not states['c']:
        exemplar_size = 0
    else:
...
def mult_terms(rel_a, irrel_a, bias_a, rel_b, irrel_b, bias_b, states, state_key_a, state_key_b, t, bias_in_rel):
    rel = 0
