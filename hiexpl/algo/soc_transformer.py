...
from bert.run_classifier import BertTokenizer, TensorDataset, SequentialSampler, DataLoader
...
def get_data_iterator_bert(tree_path, tokenizer, max_seq_length, batch_size, label_vocab=None):
    if args.task == 'sst':
        eval_examples = get_examples_sst(tree_path, train_lm=False, bert_tokenizer=tokenizer)
    elif args.task == 'yelp':
    ...
    if args.task != 'tacred':
        ...
        all_label_ids = torch.tensor([label_vocab.stoi[f.label_id] for f in eval_features], dtype=torch.long)
...
class SOCForTransformer(ExplanationBaseForTransformer):
    def __init__(self, target_model, lm_model, vocab, tree_path, output_path, config):
        ...
        self.bert_lm = None
        if self.use_bert_lm:
            self.bert_lm = BertForPreTraining.from_pretrained('bert-base-uncased', cache_dir='./bert/cache').to(
                self.gpu)
