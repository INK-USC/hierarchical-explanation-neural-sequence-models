...
class LSTMMeanRE(LSTMSentiment):
    def __init__(self, config, match_length=False):
        ...
        self.pos_embed = nn.Embedding(config.pos_size, config.offset_emb_dim, padding_idx=1)
        self.ner_embed = nn.Embedding(config.ner_size, config.offset_emb_dim, padding_idx=1)
        self.lstm = LSTMLayer(self.emb_dim + config.offset_emb_dim * 2, self.hidden_dim, 1, 0.5, True)
    ...
    def init_weights(self):
        ...
        init.xavier_uniform_(self.hidden_to_label.weight, gain=1) # initialize linear layer
...
class LSTMLayer(nn.Module):
    ...
    def __init__(self, emb_dim, hidden_dim, num_layers, dropout, use_cuda):
        ...
        self.rnn = nn.LSTM(emb_dim, hidden_dim, num_layers, batch_first=False, dropout=dropout)
